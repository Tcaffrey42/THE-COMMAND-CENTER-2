...
/* V2.15 CC CommandCenter Brand Upgrade */
.brand{background:radial-gradient(circle at 18% 12%,rgba(255,255,255,.18),transparent 28%),linear-gradient(145deg,#050505 0%,#111827 48%,#030712 100%);border:1px solid rgba(255,255,255,.10)}
.brand:before{content:"";display:block;position:absolute;inset:10px;border:1px solid rgba(255,255,255,.08);border-radius:18px;pointer-events:none}
.brandLogo{width:52px;height:52px;border-radius:18px;border:1px solid rgba(255,255,255,.32);background:linear-gradient(145deg,rgba(255,255,255,.18),rgba(255,255,255,.035));box-shadow:inset 0 0 0 1px rgba(255,255,255,.14),0 16px 34px rgba(0,0,0,.30);font-size:18px;letter-spacing:-1px}
.brandLogo span{font-weight:1000;letter-spacing:-2px}
.brandWord{color:#fff;letter-spacing:1.6px}.brand h1{font-size:31px}.brandPromise{font-size:20px!important;line-height:1.08!important;font-weight:1000!important;letter-spacing:-.04em!important;color:#fff!important;margin:2px 0 10px!important}.brandTag{background:rgba(255,255,255,.10);box-shadow:0 10px 28px rgba(0,0,0,.22)}
.top{background:rgba(255,255,255,.74);border:1px solid var(--line);border-radius:28px;padding:16px 18px;box-shadow:0 12px 38px rgba(0,0,0,.045);backdrop-filter:blur(8px)}
.title h2{letter-spacing:-.04em}.deployBadge{display:none}
.aihero,.missionHero,.amHero,.escalationHero{background:radial-gradient(circle at 10% 10%,rgba(255,255,255,.16),transparent 28%),linear-gradient(135deg,#030712,#111827 52%,#09090b)}

/* V2.15.1 Colored CC Logo Restore — actual color brought into project */
:root{
  --cc-cyan:#22d3ee;
  --cc-blue:#2563eb;
  --cc-purple:#7c3aed;
  --cc-orange:#f97316;
  --cc-gradient:linear-gradient(135deg,var(--cc-cyan) 0%,var(--cc-blue) 38%,var(--cc-purple) 72%,var(--cc-orange) 100%);
}
.brand{
  background:
    radial-gradient(circle at 18% 10%,rgba(34,211,238,.28),transparent 28%),
    radial-gradient(circle at 88% 18%,rgba(249,115,22,.22),transparent 24%),
    linear-gradient(145deg,#020617 0%,#0f172a 46%,#050816 100%);
  border:1px solid rgba(34,211,238,.24);
  box-shadow:0 20px 60px rgba(37,99,235,.18),0 18px 50px rgba(0,0,0,.22);
}
.brandLogo{
  width:58px;height:58px;border-radius:20px;padding:0;border:0;background:transparent;
  box-shadow:0 0 0 1px rgba(255,255,255,.16),0 16px 34px rgba(37,99,235,.30),0 0 34px rgba(124,58,237,.26);
  overflow:hidden;
}
.brandLogo img{width:100%;height:100%;display:block}
.brandWord{
  color:transparent!important;background:var(--cc-gradient);-webkit-background-clip:text;background-clip:text;
  text-shadow:0 0 26px rgba(34,211,238,.25);
}
.brandPromise{
  color:transparent!important;background:linear-gradient(90deg,#fff 0%,#e0f2fe 35%,#c4b5fd 70%,#fed7aa 100%);-webkit-background-clip:text;background-clip:text;
}
.brandTag{
  border-color:rgba(34,211,238,.34);background:linear-gradient(135deg,rgba(34,211,238,.14),rgba(124,58,237,.14),rgba(249,115,22,.12));
}
.kicker{color:transparent;background:var(--cc-gradient);-webkit-background-clip:text;background-clip:text}
.btn.dark{background:var(--cc-gradient);color:#fff;border:0;box-shadow:0 10px 24px rgba(37,99,235,.24)}
.nav button.active,.nav button:hover{background:var(--cc-gradient);color:#fff;box-shadow:0 10px 24px rgba(37,99,235,.18)}
.aihero,.missionHero,.amHero,.escalationHero{
  background:
    radial-gradient(circle at 12% 10%,rgba(34,211,238,.24),transparent 28%),
    radial-gradient(circle at 90% 20%,rgba(249,115,22,.20),transparent 26%),
    linear-gradient(135deg,#020617,#111827 50%,#09090b);
}

/* V2.15.3 approved chrome/blue CC CommandCenter brand upgrade */
:root{--cc-blue:#00a8ff;--cc-blue2:#006cff;--cc-dark:#020712;--cc-glow:rgba(0,168,255,.45)}
.ccLoader{position:fixed;inset:0;z-index:9999;display:grid;place-items:center;background:radial-gradient(circle at 50% 20%,rgba(0,140,255,.22),transparent 36%),linear-gradient(135deg,#02050b 0%,#07101d 52%,#02050b 100%);transition:opacity .45s ease,visibility .45s ease;color:#fff}
.ccLoader:before{content:"";position:absolute;inset:0;background-image:url('./cc-commandcenter-approved-banner.png');background-size:cover;background-position:center;opacity:.18;filter:blur(1px)}
.ccLoaded .ccLoader{opacity:0;visibility:hidden;pointer-events:none}
.ccLoaderCard{position:relative;width:min(390px,88vw);padding:34px 30px 28px;text-align:center;border:1px solid rgba(0,168,255,.28);border-radius:28px;background:rgba(2,7,18,.78);box-shadow:0 0 65px rgba(0,120,255,.32),inset 0 0 34px rgba(0,168,255,.08);backdrop-filter:blur(8px)}
.ccLoaderLogo{width:min(255px,70vw);height:auto;display:block;margin:0 auto 12px;filter:drop-shadow(0 0 18px rgba(0,168,255,.55))}
.ccLoaderText{font-weight:900;letter-spacing:.2px;margin:8px 0 18px;color:#f8fafc;text-shadow:0 0 14px rgba(0,168,255,.45)}
.ccLoaderBar{height:9px;border-radius:999px;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.32);overflow:hidden}
.ccLoaderBar span{display:block;height:100%;width:72%;border-radius:999px;background:linear-gradient(90deg,#006cff,#00d4ff);box-shadow:0 0 18px rgba(0,212,255,.8)}
.ccLoaderPct{font-size:12px;margin-top:9px;color:#dbeafe;font-weight:800}
.brand{background:radial-gradient(circle at 30% 0%,rgba(0,168,255,.24),transparent 38%),linear-gradient(145deg,#020712,#07111f 58%,#020712);border:1px solid rgba(0,168,255,.28);box-shadow:0 18px 50px rgba(0,0,0,.20),0 0 26px rgba(0,168,255,.12)}
.approvedBrand{display:block;margin:0 0 14px}.brandHeroLogo{width:100%;max-width:216px;height:auto;display:block;margin:0 auto;filter:drop-shadow(0 0 14px rgba(0,168,255,.38))}
.brandTag{border-color:rgba(0,168,255,.38);background:rgba(0,122,255,.12);box-shadow:inset 0 0 18px rgba(0,168,255,.08)}
.kicker{color:#0369a1}.title h2{letter-spacing:-1.2px}.btn.dark{background:linear-gradient(135deg,#020712,#082f49);box-shadow:0 0 0 1px rgba(0,168,255,.18) inset}
#clientSelect,#loginUserSelect{max-width:184px;min-width:0;font-size:11px;line-height:1.05;padding:9px 10px;border-radius:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:900}
#clientSelect{max-width:170px;background:linear-gradient(180deg,#ffffff,#f8fafc)}
.controls{align-items:flex-start}.deployBadge{display:none!important}
.missionHero,.aihero{background:radial-gradient(circle at 20% 0%,rgba(0,168,255,.24),transparent 35%),linear-gradient(135deg,#020712,#07111f 60%,#020712)!important;border:1px solid rgba(0,168,255,.20);box-shadow:0 22px 60px rgba(0,0,0,.22),0 0 24px rgba(0,168,255,.10)}
@media(max-width:1000px){#clientSelect,#loginUserSelect{max-width:100%;width:100%}.brandHeroLogo{max-width:260px}}


/* V2.16 Auth + Roles */
body:not(.authReady) #appShell{display:none}
body.authReady #authGate{display:none}
.authGate{min-height:100vh;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at top,#27272a 0,#09090b 48%,#000 100%);color:white}
.authCard{width:min(460px,94vw);background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.16);border-radius:30px;padding:28px;box-shadow:0 30px 90px rgba(0,0,0,.4);backdrop-filter:blur(12px)}
.authLogo{width:170px;max-width:65%;display:block;margin:0 auto 18px}.authCard h1{font-size:40px;margin:6px 0 8px;letter-spacing:-1px}.authCard p{color:#d4d4d8;line-height:1.45}.authCard input{width:100%;margin:8px 0;color:#09090b}.authBtn{width:100%;margin-top:10px}.authMessage{min-height:22px;color:#fecaca;font-weight:900;margin-top:12px}.authHint{font-size:12px;color:#d4d4d8;border-top:1px solid rgba(255,255,255,.14);margin-top:18px;padding-top:14px;line-height:1.4}.roleLock{opacity:.35;pointer-events:none}.hiddenByRole{display:none!important}

/* V2.17 AI Work Order Copilot */
.aiCopilotTile{background:linear-gradient(180deg,#f8fbff,#ffffff);border:1px solid rgba(37,99,235,.22);box-shadow:0 14px 36px rgba(15,23,42,.08)}
.aiCopilotHead{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}.aiCopilotHead h3{margin:4px 0 6px}.aiCopilotActions{display:flex;flex-wrap:wrap;gap:8px;margin:12px 0}.aiCopilotActions .btn{padding:10px 12px;border-radius:13px}.aiCopilotOutput{white-space:pre-wrap;background:#07111f;color:#eef6ff;border-radius:18px;padding:16px;line-height:1.45;min-height:180px;overflow:auto;border:1px solid rgba(255,255,255,.08);font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono",monospace}.aiCopilotOutput::selection{background:#2563eb;color:#fff}
@media(max-width:760px){.aiCopilotHead{display:block}.aiCopilotActions .btn{width:100%}}
